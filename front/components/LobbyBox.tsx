  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-4 rounded-3xl shadow-md border border-gray-200 dark:border-gray-700 h-full flex flex-col">
      <h3 className="text-lg font-semibold mb-4 text-gray-800 dark:text-white flex items-center gap-2">
        <span>🚗</span> ทริปที่เดินทางด้วยกันได้
      </h3>
        ...
        {MOCK_TRIPS.map(trip => (
          <div key={trip.id} className="p-3 border border-gray-100 dark:border-gray-700 rounded-lg hover:shadow-md transition-shadow bg-gray-50 dark:bg-gray-700/50">
            <div className="flex justify-between items-start mb-2">
              ...
              <span className="text-xs bg-green-100 text-green-800 px-2 py-0.5 rounded-full">{trip.seats} ที่นั่ง</span>
            </div>
            <div className="text-sm text-gray-600 dark:text-gray-300 space-y-1">
              <div className="flex items-center gap-1">
                <span className="text-xs text-gray-400">จาก:</span> {trip.origin}
              </div>
              <div className="flex items-center gap-1">
                <span className="text-xs text-gray-400">ถึง:</span> {trip.destination}
              </div>
              <div className="flex items-center gap-1">
                <span className="text-xs text-gray-400">เวลา:</span> {trip.time}
              </div>
            </div>
