...
router.post('/login', async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const token = jwt.sign(
            { id: user.id, email: user.email, username: user.username, role: user.role, gender: user.gender, status: userStatus },
            process.env.JWT_SECRET || 'fallback_secret',
            { expiresIn: '7d' }
        );

        res.json({
            user: { id: user.id, username: user.username, email: user.email, role: user.role, gender: user.gender, created_at: user.created_at, status: userStatus },
            token,
        });
    } catch (err) {
